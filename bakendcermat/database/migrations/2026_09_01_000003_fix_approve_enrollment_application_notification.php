...
/**
 * BUG EN PRODUCCION: approve_enrollment_application aborta al aprobar la
 * matricula de un alumno cuyo apoderado YA tiene cuenta de usuario.
 *
 * El paso 7 de la funcion inserta una notificacion con type
 * 'matricula_aprobada' y status 'unread'. Ninguno de los dos valores existe:
 * el enum notification_type no tiene 'matricula_aprobada', y
 * notification_status es ('no_leida', 'leida'), no ('unread', 'read').
 * Postgres corta con 22P02 (invalid input value for enum) y, por estar dentro
 * de la funcion, revierte TODA la aprobacion.
 *
 * No se notaba porque los apoderados nuevos se crean con user_id NULL y la
 * rama ni se ejecuta. Pero 12 de los 32 apoderados ya tienen cuenta, asi que
 * hoy es imposible matricular a un hermano de cualquiera de ellos.
 *
 * Este archivo reemplaza la funcion con la MISMA definicion, cambiando
 * unicamente esas dos lineas (verificado con diff: 2 lineas de 170):
 *   'matricula_aprobada' -> 'comunicado_nuevo'  (valor real del enum, es el
 *                                                aviso institucional generico)
 *   'unread'             -> 'no_leida'          (valor real de notification_status)
 *
 * El resto del cuerpo se transcribe literal desde pg_get_functiondef() para
 * no alterar por accidente la creacion de apoderado/alumno/matricula ni el
 * trigger de inscripcion automatica a cursos.
 */
return new class extends Migration
{
    ...
    {
        DB::unprepared(<<<'SQL'
CREATE OR REPLACE FUNCTION public.approve_enrollment_application(p_application_id uuid, p_section_id uuid, p_approved_by uuid)
 RETURNS TABLE(success boolean, message text, student_id uuid, guardian_id uuid, user_id uuid)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
...
SQL);
    ...
    {
        // Restaura la definicion original, con el bug incluido: revertir una
        // migracion debe devolver la base al estado exacto que tenia, no a
        // uno "mejor". Si se revierte esto, vuelve a fallar la aprobacion del
        // segundo hijo de un apoderado con cuenta.
        DB::unprepared(<<<'SQL'
CREATE OR REPLACE FUNCTION public.approve_enrollment_application(p_application_id uuid, p_section_id uuid, p_approved_by uuid)
 RETURNS TABLE(success boolean, message text, student_id uuid, guardian_id uuid, user_id uuid)
 LANGUAGE plpgsql
