...
\restrict e1JiUFibnFBVLPn4rJvtGMZBGYWvn2xk7TJwAq1kS0y1U8r99swdVXT5OCQbV3f

-- Dumped from database version 17.6
-- Dumped by pg_dump version 17.6

-- Started on 2026-03-14 21:28:04
...
BEGIN
  ...
  -- Verificar que la secciÃ³n corresponde al grado solicitado
  IF NOT EXISTS (
    SELECT 1 FROM sections s
    WHERE s.id = p_section_id AND s.grade_level_id = v_application.grade_level_id
  ) THEN
    RETURN QUERY SELECT false, 'La secciÃ³n no corresponde al grado solicitado'::TEXT, NULL::UUID, NULL::UUID, NULL::UUID;
    RETURN;
  END IF;
  
  -- Obtener aÃ±o acadÃ©mico activo
  ...
  IF v_academic_year_id IS NULL THEN
    RETURN QUERY SELECT false, 'No hay aÃ±o acadÃ©mico activo'::TEXT, NULL::UUID, NULL::UUID, NULL::UUID;
    RETURN;
  END IF;
  ...
  IF v_guardian_id IS NULL THEN
    -- Crear apoderado SIN usuario (se crearÃ¡ despuÃ©s desde el frontend/admin)
    -- La creaciÃ³n de usuarios de Supabase Auth debe hacerse desde el API, no desde SQL
    INSERT INTO guardians (
      ...
      NULL,  -- user_id se asignarÃ¡ cuando el admin cree el usuario manualmente
  ...
  -- 2. Generar cÃ³digo de estudiante
  ...
  -- NOTA: La creaciÃ³n del usuario de Supabase Auth debe hacerse desde el frontend
  -- porque no tenemos acceso directo a auth.users desde funciones SQL.
  -- El frontend deberÃ¡ llamar a supabase.auth.admin.createUser() despuÃ©s de esta funciÃ³n.
  
  -- 3. Crear estudiante (sin user_id, se asignarÃ¡ despuÃ©s desde el frontend)
  INSERT INTO students (
    ...
    NULL,  -- user_id se asignarÃ¡ cuando el admin cree el usuario manualmente o desde frontend
    p_section_id,
    v_student_code,
  ...
  -- 4. Crear relaciÃ³n estudiante-apoderado
  ...
  -- (El trigger auto_enroll_student_to_section_courses deberÃ­a ejecutarse automÃ¡ticamente)
  PERFORM pg_sleep(0.1); -- PequeÃ±a pausa para asegurar que el trigger se complete
  
  -- 7. Crear notificaciÃ³n para el apoderado (solo si tiene usuario)
  ...
  IF v_user_id IS NOT NULL THEN
    INSERT INTO notifications (
      ...
      'matricula_aprobada',
      'MatrÃ­cula Aprobada',
      'La solicitud de matrÃ­cula para ' || v_application.student_first_name || ' ' || v_application.student_last_name || ' ha sido aprobada. CÃ³digo: ' || v_student_code,
  ...
  -- Retornar Ã©xito con IDs
  RETURN QUERY SELECT true, 'MatrÃ­cula aprobada exitosamente. Estudiante inscrito en ' || 
...
COMMENT ON FUNCTION public.approve_enrollment_application(p_application_id uuid, p_section_id uuid, p_approved_by uuid) IS 'Aprueba una solicitud de matrÃ­cula, crea estudiante y apoderado, y usa student_guardians en lugar de guardian_id directo. El trigger auto_enroll_student_to_section_courses inscribe automÃ¡ticamente al estudiante en los cursos de su secciÃ³n.';
...
BEGIN
  IF TG_OP = 'UPDATE' THEN
    -- Solo auditar si cambiÃ³ el grado o el estado
    IF OLD.grade IS DISTINCT FROM NEW.grade OR OLD.status IS DISTINCT FROM NEW.status THEN
      ...
      -- Si se publicÃ³, notificar al estudiante y apoderados
      IF OLD.status = 'borrador' AND NEW.status = 'publicada' THEN
        ...
        -- Solo crear notificaciÃ³n si el estudiante tiene user_id
        IF v_student_user_id IS NOT NULL THEN
          PERFORM create_notification(
            v_student_user_id,
            'evaluacion_publicada'::notification_type,
            'Nueva calificaciÃ³n publicada',
            'Se ha publicado una nueva calificaciÃ³n en ' || (SELECT name FROM courses WHERE id = NEW.course_id),
            'evaluation',
            NEW.id
          );
        END IF;
...
DECLARE
  ...
  v_course RECORD;
BEGIN
  -- Solo procesar cuando se asigna o cambia de secciÃ³n (y el estudiante estÃ¡ activo)
  IF (NEW.section_id IS NOT NULL AND NEW.status = 'active') AND 
    ...
    IF v_academic_year_id IS NULL THEN
      RAISE EXCEPTION 'No hay aÃ±o acadÃ©mico activo';
    END IF;
    
    -- Si cambiÃ³ de secciÃ³n, marcar cursos antiguos como dropped
    ...
    -- Inscribir en todos los cursos asignados a la nueva secciÃ³n
    FOR v_course IN
...
COMMENT ON FUNCTION public.auto_enroll_student_to_section_courses() IS 'Auto-inscribe al estudiante en todos los cursos de su secciÃ³n cuando se matricula';
...
DECLARE
  ...
  v_result JSON;
BEGIN
  -- Nota: La creaciÃ³n real del usuario en auth.users debe hacerse
  -- desde el servidor con la API de Supabase Auth (no desde SQL)
  -- Esta funciÃ³n solo prepara la estructura
...
BEGIN
  IF NEW.slug IS NULL OR NEW.slug = '' THEN
    -- Intentar usar unaccent si estÃ¡ disponible, sino usar el tÃ­tulo directamente
    BEGIN
    ...
    -- Limpiar el slug: minÃºsculas, solo alfanumÃ©ricos y guiones
    NEW.slug := lower(
      regexp_replace(
        regexp_replace(
          base_slug,
    ...
    IF NEW.status IN ('aprobada', 'rechazada') AND NEW.guardian_id IS NOT NULL THEN
      ...
      IF v_guardian_user_id IS NOT NULL THEN
        PERFORM create_notification(
          ...
          END,
          'JustificaciÃ³n ' || NEW.status,
          'Su justificaciÃ³n de inasistencia ha sido ' || NEW.status,
          'justification',
          NEW.id
        );
      END IF;
...
DECLARE
  v_student RECORD;
BEGIN
  -- Solo procesar cuando se crea o activa una asignaciÃ³n
  IF NEW.is_active = TRUE AND (OLD IS NULL OR OLD.is_active = FALSE) THEN
    
    -- Inscribir a todos los estudiantes activos de esa secciÃ³n en el curso
    FOR v_student IN
      SELECT id
      FROM students
      WHERE section_id = NEW.section_id
  ...
  -- Si se desactiva la asignaciÃ³n, marcar inscripciones como dropped
...
COMMENT ON FUNCTION public.sync_student_enrollments_on_teacher_assignment() IS 'Sincroniza inscripciones cuando se asigna un nuevo curso a una secciÃ³n';
...
BEGIN
  ...
  IF current_course_count >= max_courses_per_teacher THEN
    RAISE EXCEPTION 'El docente ya tiene % cursos asignados. MÃ¡ximo permitido: %', 
...
COMMENT ON TABLE public.course_schedules IS 'Horarios de clases por secciÃ³n, curso y docente';
...
COMMENT ON COLUMN public.course_schedules.day_of_week IS '1=Lunes, 2=Martes, 3=MiÃ©rcoles, 4=Jueves, 5=Viernes, 6=SÃ¡bado, 7=Domingo';
...
COMMENT ON COLUMN public.course_schedules.room_number IS 'NÃºmero o nombre del aula/salÃ³n';
...
COMMENT ON COLUMN public.courses.color IS 'Color hexadecimal para identificaciÃ³n visual del curso (#RRGGBB)';
...
COMMENT ON COLUMN public.profiles.updated_at IS 'Ãšltima actualizaciÃ³n del perfil';
...
COMMENT ON COLUMN public.profiles.created_by IS 'Usuario administrador que creÃ³ este perfil';
...
CREATE TABLE public.public_news (
    ...
    author text DEFAULT 'DirecciÃ³n General'::text,
...
COMMENT ON TABLE public.student_course_enrollments IS 'Inscripciones de estudiantes a cursos especÃ­ficos';
...
f7962b8d-64a7-4870-9a79-845e113668a8	f7962b8d-64a7-4870-9a79-845e113668a8	{"sub": "f7962b8d-64a7-4870-9a79-845e113668a8", "role": "student", "email": "emerson.cuna@cermatschool.edu.pe", "full_name": "Emerson CuÃ±a", "email_verified": false, "phone_verified": false}	email	2025-12-22 17:35:14.94728+00	2025-12-22 17:35:14.947336+00	2025-12-22 17:35:14.947336+00	03d7b4bd-35ee-4e57-827e-91eded221868
14c0d2ba-a330-4436-9be8-58d75eb5947a	14c0d2ba-a330-4436-9be8-58d75eb5947a	{"sub": "14c0d2ba-a330-4436-9be8-58d75eb5947a", "role": "guardian", "email": "lorenzo@gmail.com", "full_name": "Lorenzo CuÃ±a", "email_verified": false, "phone_verified": false}	email	2025-12-22 17:35:15.94508+00	2025-12-22 17:35:15.945767+00	2025-12-22 17:35:15.945767+00	5affaf5c-221c-4efe-b150-35c07b2885c8
36715e53-ad41-4f26-b5da-43323d7a92fa	36715e53-ad41-4f26-b5da-43323d7a92fa	{"sub": "36715e53-ad41-4f26-b5da-43323d7a92fa", "role": "student", "email": "estudiante.cermat@cermatschool.edu.pe", "full_name": "Estudiante Cermat", "email_verified": false, "phone_verified": false}	email	2026-03-12 15:50:01.796822+00	2026-03-12 15:50:01.79688+00	2026-03-12 15:50:01.79688+00	f5c5406d-6989-440f-8cff-b1752451024c
7dfcb4c2-fef0-4088-815a-6ac1c80c5cff	7dfcb4c2-fef0-4088-815a-6ac1c80c5cff	{"sub": "7dfcb4c2-fef0-4088-815a-6ac1c80c5cff", "role": "guardian", "email": "apoderadocermat1@gmail.com", "full_name": "Apoderado Cermat", "email_verified": false, "phone_verified": false}	email	2026-03-12 15:50:03.357712+00	2026-03-12 15:50:03.357754+00	2026-03-12 15:50:03.357754+00	b0d97db2-42b0-45c9-b0f5-2cd5b4a82de2
5cb4cf8d-d20c-4298-b2b2-360f0121a11e	5cb4cf8d-d20c-4298-b2b2-360f0121a11e	{"sub": "5cb4cf8d-d20c-4298-b2b2-360f0121a11e", "role": "student", "email": "mario.cervantes@cermatschool.edu.pe", "full_name": "Mario Cervantes", "email_verified": false, "phone_verified": false}	email	2026-03-15 01:26:32.67304+00	2026-03-15 01:26:32.673098+00	2026-03-15 01:26:32.673098+00	5b70bf17-463a-4810-96da-9e6fb0c7516e
7d40a080-4e6f-4034-8724-74f5f6a93e8d	7d40a080-4e6f-4034-8724-74f5f6a93e8d	{"sub": "7d40a080-4e6f-4034-8724-74f5f6a93e8d", "role": "guardian", "email": "mariosr@gmail.com", "full_name": "MarioSr Cervantes", "email_verified": false, "phone_verified": false}	email	2026-03-15 01:26:33.898489+00	2026-03-15 01:26:33.89853+00	2026-03-15 01:26:33.89853+00	f28d3739-7a2a-438d-854e-3148b79d6562
\.
...
184c8864-0d7c-4ad1-b9a2-29238cb4b098	2026-03-13 03:40:27.109953+00	2026-03-13 03:40:27.109953+00	password	57370e49-b937-451b-ab72-93ab3eead711
e4b518a1-d539-493b-8b3d-b50150d94e87	2026-03-13 03:41:18.458058+00	2026-03-13 03:41:18.458058+00	password	0e567c64-ae15-4c21-9184-46ace2fe2f82
...
72fb189a-0eeb-4c00-8ba0-05f739514c5d	2026-03-15 01:26:33.905003+00	2026-03-15 01:26:33.905003+00	password	ce2ef920-1d50-48da-b185-50500de36ac9
d2b28dfb-5bc9-496c-a1ba-f83938cd6fe6	2026-03-15 01:41:53.093167+00	2026-03-15 01:41:53.093167+00	password	5c82e327-e0ae-4f01-9eb9-c33266b1afa8
dac21ae9-a731-416a-b301-dd4f2fc9af72	2026-03-15 01:41:56.104235+00	2026-03-15 01:41:56.104235+00	password	fb49cef6-fbff-4d54-81d1-396296aa173a
4a570d02-f0a6-4668-b323-a7e96bcb21ad	2026-03-15 01:53:34.493863+00	2026-03-15 01:53:34.493863+00	password	01bbbeaa-4823-47c5-b0b5-c8a0cc3ef916
5cdde95d-4a39-4f17-a8e0-c2901ffb87b2	2026-03-15 01:53:35.579304+00	2026-03-15 01:53:35.579304+00	password	317f15a9-a5fc-498d-8340-a6732120f70c
70489f75-a279-4d3c-b411-0fa6b5e0aeac	2026-03-12 15:50:01.815321+00	2026-03-12 15:50:01.815321+00	password	70512c4e-c7a1-4d16-bcbe-254a9f4a417e
\.
...
00000000-0000-0000-0000-000000000000	465	pkefoxu5ksm6	7d40a080-4e6f-4034-8724-74f5f6a93e8d	f	2026-03-15 01:26:33.903928+00	2026-03-15 01:26:33.903928+00	\N	72fb189a-0eeb-4c00-8ba0-05f739514c5d
...
00000000-0000-0000-0000-000000000000	473	ruxcfwdhjsse	381ea57b-c0d5-4f1d-a5fd-57c02fe2c3a8	f	2026-03-15 01:41:53.09198+00	2026-03-15 01:41:53.09198+00	\N	d2b28dfb-5bc9-496c-a1ba-f83938cd6fe6
00000000-0000-0000-0000-000000000000	474	7tqstkpr5nvv	381ea57b-c0d5-4f1d-a5fd-57c02fe2c3a8	f	2026-03-15 01:41:56.103101+00	2026-03-15 01:41:56.103101+00	\N	dac21ae9-a731-416a-b301-dd4f2fc9af72
00000000-0000-0000-0000-000000000000	475	dp253chlgeck	381ea57b-c0d5-4f1d-a5fd-57c02fe2c3a8	f	2026-03-15 01:53:34.478056+00	2026-03-15 01:53:34.478056+00	\N	4a570d02-f0a6-4668-b323-a7e96bcb21ad
00000000-0000-0000-0000-000000000000	476	acd5sbyxngy5	381ea57b-c0d5-4f1d-a5fd-57c02fe2c3a8
mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/145.0.0.0 Safari/537.36 Avast/145.0.34156.160	179.7.0.160	\N	\N	\N	\N	\N
\.
...
00000000-0000-0000-0000-000000000000	4ac1dd30-d8b6-4bc2-b508-1b618070cc51	authenticated	authenticated	jperez@cermatschool.edu.pe	$2a$10$ekMInt5KBD/VQ91b/pHkp.D8bBHID7iTkGUaJ76pmsxkGMBxNd/DW	2025-12-05 04:12:27.293173+00	\N		\N		\N			\N	2026-03-13 03:41:18.45521+00	{"provider": "email", "providers": ["email"]}	{"email_verified": true}	\N	2025-12-05 04:12:27.285931+00	2026-03-13 05:25:52.113264+00	\N	\N			\N		0	\N		\N	f	\N	f
...
00000000-0000-0000-0000-000000000000	7d71be34-f728-47ce-9c1a-0a4b136809fc	authenticated	authenticated	yasmin.yucramamani@cermatschool.edu.pe	$2a$10$a8VgxyjTAEiAQvI46qiSn.ZkbJzr7vcteV2i/Byn8TS8ppQLwwR9O	2025-12-09 22:57:29.739386+00	\N		\N		\N			\N	2026-03-13 03:35:25.475674+00	{"provider": "email", "providers": ["email"]}	{"sub": "7d71be34-f728-47ce-9c1a-0a4b136809fc", "role": "student", "email": "yasmin.yucramamani@cermatschool.edu.pe", "full_name": "yasmin yucra mamani", "email_verified": true, "phone_verified": false}	\N	2025-12-09 22:57:29.718406+00	2026-03-13 03:35:25.478372+00	\N	\N			\N		0	\N		\N	f	\N	f
...
00000000-0000-0000-0000-000000000000	381ea57b-c0d5-4f1d-a5fd-57c02fe2c3a8	authenticated	authenticated	admin@cermatschool.edu.pe	$2a$10$oqApiTRVsii6lSJs7aIJCed5rQN/S6wLAuk/iu2uyptVYUBV3lxpy	2025-12-05 03:55:31.840684+00	\N		\N		\N			\N	2026-03-15 01:53:35.577004+00	{"provider": "email", "providers": ["email"]}	{"email_verified": true}	\N	2025-12-05 03:55:31.819513+00	2026-03-15 01:53:35.578996+00	\N	\N			\N		0	\N		\N	f	\N	f
...
00000000-0000-0000-0000-000000000000	471c0d0e-b6fc-4487-b8c2-1eb1ccebec9b	authenticated	authenticated	antonio.lara@cermatschool.edu.pe	$2a$10$HN.kLDddfubczHJoPjJYne788zLUNApaqpnqMuKxWQubj05pb5ykC	2025-12-19 19:39:35.59196+00	\N		\N		\N			\N	2026-03-13 19:43:13.292166+00	{"provider": "email", "providers": ["email"]}	{"sub": "471c0d0e-b6fc-4487-b8c2-1eb1ccebec9b", "role": "student", "email": "antonio.lara@cermatschool.edu.pe", "full_name": "Antonio  Lara", "email_verified": true, "phone_verified": false}	\N	2025-12-19 19:39:35.554989+00	2026-03-13 19:43:13.295091+00	\N	\N			\N		0	\N		\N	f	\N	f
...
00000000-0000-0000-0000-000000000000	5cb4cf8d-d20c-4298-b2b2-360f0121a11e	authenticated	authenticated	mario.cervantes@cermatschool.edu.pe	$2a$10$2CpZi0BFeenk8FVEsiOe7O30YicQbrs5LbK.Q7hvPQo8npvTlHCqK	2026-03-15 01:26:32.681123+00	\N		\N		\N			\N	2026-03-15 01:41:18.883447+00	{"provider": "email", "providers": ["email"]}	{"sub": "5cb4cf8d-d20c-4298-b2b2-360f0121a11e", "role": "student", "email": "mario.cervantes@cermatschool.edu.pe", "full_name": "Mario Cervantes", "email_verified": true, "phone_verified": false}	\N	2026-03-15 01:26:32.661609+00	2026-03-15 01:41:18.886942+00	\N	\N			\N		0	\N		\N	f	\N	f
00000000-0000-0000-0000-000000000000	f7962b8d-64a7-4870-9a79-845e113668a8	authenticated	authenticated	emerson.cuna@cermatschool.edu.pe	$2a$10$EAeiWcEyRNaYkVAHw0mXYeoJYa1j2mTcgHN9KE0JkxZ.d6IyJWiJG	2025-12-22 17:35:14.954353+00	\N		\N		\N			\N	2025-12-22 19:01:35.30167+00	{"provider": "email", "providers": ["email"]}	{"sub": "f7962b8d-64a7-4870-9a79-845e113668a8", "role": "student", "email": "emerson.cuna@cermatschool.edu.pe", "full_name": "Emerson CuÃ±a", "email_verified": true, "phone_verified": false}	\N	2025-12-22 17:35:14.936107+00	2025-12-22 19:01:35.304481+00	\N	\N			\N		0	\N		\N	f	\N	f
00000000-0000-0000-0000-000000000000	7d40a080-4e6f-4034-8724-74f5f6a93e8d	authenticated	authenticated	mariosr@gmail.com	$2a$10$BFTl58xGJuzlbz277EQnw.aLmgT6f2VUlHvT0UwvmeVfQ7KGRCfiW	2026-03-15 01:26:33.900401+00	\N		\N		\N			\N	2026-03-15 01:26:33.902512+00	{"provider": "email", "providers": ["email"]}	{"sub": "7d40a080-4e6f-4034-8724-74f5f6a93e8d", "role": "guardian", "email": "mariosr@gmail.com", "full_name": "MarioSr Cervantes", "email_verified": true, "phone_verified": false}	\N	2026-03-15 01:26:33.895087+00	2026-03-15 01:26:33.904734+00	\N	\N			\N		0	\N		\N	f	\N	f
...
00000000-0000-0000-0000-000000000000	36715e53-ad41-4f26-b5da-43323d7a92fa	authenticated	authenticated	estudiante.cermat@cermatschool.edu.pe	$2a$10$8vJ7zPEmnihu/HIGn9JvYumVsymG1mSkZYffJ.fjb60yuW6dwoWs6	2026-03-12 15:50:01.803032+00	\N		\N		\N			\N	2026-03-12 15:50:01.811466+00	{"provider": "email", "providers": ["email"]}	{"sub": "36715e53-ad41-4f26-b5da-43323d7a92fa", "role": "student", "email": "estudiante.cermat@cermatschool.edu.pe", "full_name": "Estudiante Cermat", "email_verified": true, "phone_verified": false}	\N	2026-03-12 15:50:01.77742+00	2026-03-12 15:50:01.814338+00	\N	\N			\N		0	\N		\N	f	\N	f
00000000-0000-0000-0000-000000000000	14c0d2ba-a330-4436-9be8-58d75eb5947a	authenticated	authenticated	lorenzo@gmail.com	$2a$10$2HM2MlvpPCVVuX7K0vR6POjkCEDmceJYnMwSdVqf6FLmDQW6AmMf2	2025-12-22 17:35:15.948851+00	\N		\N		\N			\N	2025-12-22 17:35:15.951757+00	{"provider": "email", "providers": ["email"]}	{"sub": "14c0d2ba-a330-4436-9be8-58d75eb5947a", "role": "guardian", "email": "lorenzo@gmail.com", "full_name": "Lorenzo CuÃ±a", "email_verified": true, "phone_verified": false}	\N	2025-12-22 17:35:15.941893+00	2025-12-22 17:35:15.955086+00	\N	\N			\N		0	\N		\N	f	\N	f
00000000-0000-0000-0000-000000000000	7dfcb4c2-fef0-4088-815a-6ac1c80c5cff	authenticated	authenticated	apoderadocermat1@gmail.com	$2a$10$5sPpxWjCgDdlw2KkOQMZjeEBYnsrK7ER9kbNHpVH1sXe0jn2cuQC6	2026-03-12 15:50:03.359701+00	\N		\N		\N			\N	2026-03-13 01:25:11.967773+00	{"provider": "email", "providers": ["email"]}	{"sub": "7dfcb4c2-fef0-4088-815a-6ac1c80c5cff", "role": "guardian", "email": "apoderadocermat1@gmail.com", "full_name": "Apoderado Cermat", "email_verified": true, "phone_verified": false}	\N	2026-03-12 15:50:03.350688+00	2026-03-13 01:25:11.969723+00	\N	\N			\N		0	\N		\N	f	\N	f
d6770786-270f-42e2-b885-bde9a37e2fd0	14c0d2ba-a330-4436-9be8-58d75eb5947a	Lorenzo	CuÃ±a	\N	72833443	lorenzo@gmail.com	\N	Padre	f	2025-12-22 17:35:16.530136+00	2025-12-22 17:35:16.530136+00
666d614f-5902-4cbe-851d-cf8f1f0eb6fb	\N	Apoderado	Cermat	72631456	987786767	apoderadocermat1@gmail.com	Av Infancia NÂ°300	Padre	f	2026-03-12 15:50:00.910496+00	2026-03-12 15:50:00.910496+00
deda39d3-0932-43d6-8408-d34c388c3ea5	7dfcb4c2-fef0-4088-815a-6ac1c80c5cff	Apoderado	Cermat	\N	987786767	apoderadocermat1@gmail.com	\N	Padre	f	2026-03-12 15:50:04.057225+00	2026-03-12 15:50:04.057225+00
06723ef8-af44-4e1f-91a8-34bd35862397	\N	MarioSr	Cervantes	72634341	989786756	mariosr@gmail.com	Av Infancia NÂ°300	Padre	f	2026-03-15 01:26:31.94592+00	2026-03-15 01:26:31.94592+00
6fe88f0a-caee-43db-aeea-88cce9a521d5	7d40a080-4e6f-4034-8724-74f5f6a93e8d	MarioSr	Cervantes	\N	989786756	mariosr@gmail.com	\N	Padre	f	2026-03-15 01:26:34.634288+00	2026-03-15 01:26:34.634288+00
\.
...
4ac1dd30-d8b6-4bc2-b508-1b618070cc51	teacher	Prof. Juan PÃ©rez	\N	\N	jperez@cermatschool.edu.pe	\N	t	2025-12-05 04:14:26.873411+00	2025-12-05 04:14:26.873411+00	\N	\N
21748cbb-948b-480b-8ae7-093a3accccd8	guardian	MarÃ­a GarcÃ­a	\N	\N	mgarcia@email.com	\N	t	2025-12-05 04:17:01.185974+00	2025-12-05 04:17:01.185974+00	\N	\N
...
f7962b8d-64a7-4870-9a79-845e113668a8	student	Emerson CuÃ±a	\N	\N	emerson.cuna@cermatschool.edu.pe	\N	t	2025-12-22 17:35:15.569074+00	2025-12-22 17:35:15.569074+00	\N	\N
14c0d2ba-a330-4436-9be8-58d75eb5947a	guardian	Lorenzo CuÃ±a	\N	\N	lorenzo@gmail.com	\N	t	2025-12-22 17:35:16.243479+00	2025-12-22 17:35:16.243479+00	\N	\N
...
36715e53-ad41-4f26-b5da-43323d7a92fa	student	Estudiante Cermat	\N	\N	estudiante.cermat@cermatschool.edu.pe	\N	t	2026-03-12 15:50:02.805544+00	2026-03-12 15:50:02.805544+00	\N	\N
7dfcb4c2-fef0-4088-815a-6ac1c80c5cff	guardian	Apoderado Cermat	\N	\N	apoderadocermat1@gmail.com	\N	t	2026-03-12 15:50:03.678871+00	2026-03-12 15:50:03.678871+00	\N	\N
5cb4cf8d-d20c-4298-b2b2-360f0121a11e	student	Mario Cervantes	\N	\N	mario.cervantes@cermatschool.edu.pe	\N	t	2026-03-15 01:26:33.507429+00	2026-03-15 01:26:33.507429+00	\N	\N
7d40a080-4e6f-4034-8724-74f5f6a93e8d	guardian	MarioSr Cervantes	\N	\N	mariosr@gmail.com	\N	t	2026-03-15 01:26:34.238475+00	2026-03-15 01:26:34.238475+00	\N	\N
\.
...
6a2fa4f9-c162-46e1-af83-4eb8dd21abd0	Proceso de AdmisiÃ³n Abierto	proceso-de-admision-abierto-6a2fa4f9	Las inscripciones para el proceso de admisiÃ³n 2026 estÃ¡n abiertas. Conoce los requisitos y fechas importantes.	Ofrecemos becas por rendimiento acadÃ©mico y descuentos por hermanos. No pierdas la oportunidad de formar parte de nuestra comunidad educativa.	https://images.unsplash.com/photo-1427504494785-3a9ca7044f45?w=800&h=600&fit=crop	admisiones	SecretarÃ­a AcadÃ©mica	publicado	t	2025-12-19 06:06:26.738605+00	\N	\N	2025-12-22 06:06:26.738605+00	2025-12-22 06:06:26.738605+00
b4005acf-9f78-4c59-b5e6-160ce7004351	Inicio del AÃ±o Escolar 2025	inicio-del-ano-escolar-2025-b4005acf	Damos la bienvenida a todos nuestros estudiantes y familias para un nuevo aÃ±o lleno de aprendizaje y crecimiento.	El aÃ±o escolar 2025 inicia con renovadas energÃ­as y proyectos innovadores. Nuestro compromiso con la excelencia educativa se mantiene firme, ofreciendo a nuestros estudiantes las mejores oportunidades de desarrollo integral.	https://images.unsplash.com/photo-1523050854058-8df90110c9f1?w=800&h=600&fit=crop	institucional	DirecciÃ³n General	publicado	f	2025-12-15 06:06:26.738605+00	\N	\N	2025-12-22 06:06:26.738605+00	2025-12-22 18:35:09.12858+00
1a102576-c5b5-4db2-8ae4-42e4605cece1	Talleres de RobÃ³tica 2025	talleres-de-robotica-2025-1a102576	Inauguramos nuestros nuevos talleres de robÃ³tica educativa para primaria y secundaria.	Los estudiantes podrÃ¡n desarrollar habilidades en programaciÃ³n, diseÃ±o y construcciÃ³n de robots. Este programa busca fomentar el pensamiento computacional y la creatividad en nuestros alumnos.	https://images.unsplash.com/photo-1581092160562-40aa08e78837?w=800&h=600&fit=crop	tecnologia	Ãrea de InnovaciÃ³n	publicado	t	2025-12-22 19:31:38.35+00	\N	\N	2025-12-22 06:06:26.738605+00	2025-12-22 19:31:37.296254+00
9075db78-7f4f-4ff7-83e3-4b587607b020	hola mundo	hola-mundo-9075db78	adsdasd	asdasd	https://images.unsplash.com/photo-1581092160562-40aa08e78837?w=800&h=600&fit=crop	institucional	DirecciÃ³n General	publicado	f	2025-12-22 19:31:47.591+00	\N	\N	2025-12-22 06:12:42.125494+00	2025-12-22 19:31:46.516359+00
610c21c5-43aa-4eeb-b42a-b03eb065b24f	Nothing	nothing-610c21c5	Nothing	Nothing to commit	https://images.unsplash.com/photo-1581092160562-40aa08e78837?w=800&h=600&fit=crop	institucional	DirecciÃ³n General	publicado	f	2025-12-22 19:32:22.914+00	\N	\N	2025-12-22 19:32:21.885076+00	2025-12-22 19:32:21.885076+00
\.
...
d13a9a88-e6f1-415b-b516-e1a36e8c59a8	fba8986d-1b9c-4ed7-9aa4-a09bec7b641f	6c2b34b8-3248-4332-a064-2cdfff6fe87f	00eb9249-d8ef-48d7-9ad3-e285de259bb4	82fad6ef-2963-4d3f-b42e-4d40f83929da	2026-03-12 15:50:00.910496+00	active	2026-03-12 15:50:00.910496+00	2026-03-12 15:50:00.910496+00
344b677c-0966-4a87-bdd8-bd9bb1fbbcc2	\N	EST000014	Ronald	Quispe	72344543	2025-12-22	M	Av Infancia NÂ°300	741116ee-ae2e-443f-ac98-17dc4479ad13	2025-12-22	active	\N	2025-12-22 17:04:43.134745+00	2025-12-22 17:04:43.134745+00
cc35c8f9-2e64-4345-9c04-e0c71f83ef8f	\N	EST000015	Esther	CuÃ±a	72383443	2025-12-22	M	Av Infancia NÂ°300	741116ee-ae2e-443f-ac98-17dc4479ad13	2025-12-22	active	\N	2025-12-22 17:20:21.128981+00	2025-12-22 17:20:21.128981+00
b989c2cd-fe8d-4545-8ee3-8f3676864fcb	\N	EST000016	Rosmery	CuÃ±a	78232322	2025-12-22	F	Av Infancia NÂ°300	741116ee-ae2e-443f-ac98-17dc4479ad13	2025-12-22	active	\N	2025-12-22 17:26:01.77409+00	2025-12-22 17:26:01.77409+00
9d952acf-e6b4-4da7-9c54-cf52e841fc85	\N	EST000017	Renzo	CuÃ±a	72832323	2025-12-22	M	Av Infancia NÂ°300	741116ee-ae2e-443f-ac98-17dc4479ad13	2025-12-22	active	\N	2025-12-22 17:30:33.372154+00	2025-12-22 17:30:33.372154+00
2daf9a5d-9743-479a-9acc-d2de3a338241	\N	EST000018	Emerson	CuÃ±a	72633443	2009-02-04	M	Av Infancia NÂ°300	741116ee-ae2e-443f-ac98-17dc4479ad13	2025-12-22	active	\N	2025-12-22 17:35:14.404121+00	2025-12-22 17:35:14.404121+00
fba8986d-1b9c-4ed7-9aa4-a09bec7b641f	\N	EST000019	Estudiante	Cermat	72631471	2010-01-12	M	Av Infancia NÂ°300	00eb9249-d8ef-48d7-9ad3-e285de259bb4	2026-03-12	active	\N	2026-03-12 15:50:00.910496+00	2026-03-12 15:50:00.910496+00
ff35fa8e-6b7b-4f78-a075-ab6dce05c821	\N	EST000020	Mario	Cervantes	72631490	2007-01-14	M	Av Infancia NÂ°300	00eb9249-d8ef-48d7-9ad3-e285de259bb4	2026-03-15	active	\N	2026-03-15 01:26:31.94592+00	2026-03-15 01:26:31.94592+00
\.
...
bb6fe8ff-db2a-46d2-a772-1fc6a50d20a8	43d662f7-accf-4532-a39b-582cf1e7e2bc	00eb9249-d8ef-48d7-9ad3-e285de259bb4	c72fe5f7-5a2f-4c57-8903-0ff3fdb28765	82fad6ef-2963-4d3f-b42e-4d40f83929da	381ea57b-c0d5-4f1d-a5fd-57c02fe2c3a8	2026-03-15 01:39:34.093843	t	\N	2026-03-15 01:39:34.093843	2026-03-15 01:39:34.093843
\.
...
43d662f7-accf-4532-a39b-582cf1e7e2bc	4ac1dd30-d8b6-4bc2-b508-1b618070cc51	T001	Juan	PÃ©rez	12345678	\N	\N	\N	2025-12-08	active	\N	2025-12-08 17:54:58.978512+00	2025-12-08 17:54:58.978512+00
\.
...
-- Completed on 2026-03-14 21:29:21

--
-- PostgreSQL database dump complete
--

\unrestrict e1JiUFibnFBVLPn4rJvtGMZBGYWvn2xk7TJwAq1kS0y1U8r99swdVXT5OCQbV3f
