...
@Component({
  ...
  template: `
    <div 
      class="fixed bottom-6 right-6 z-50 flex items-center gap-3 transition-all duration-300"
      [class.opacity-0]="!isVisible()"
      [class.translate-y-20]="!isVisible()"
      [class.opacity-100]="isVisible()"
      [class.translate-y-0]="isVisible()"
    >
      <!-- Globo de mensaje: a la IZQUIERDA del botón (misma altura) -->
      <div
        class="max-w-[calc(100vw-6.5rem)] sm:max-w-xs transition-all duration-300 ease-out"
        [class.opacity-0]="!showTooltip()"
        [class.opacity-100]="showTooltip()"
        [class.translate-x-4]="!showTooltip()"
        [class.translate-x-0]="showTooltip()"
        [class.pointer-events-none]="!showTooltip()"
      >
        <div class="relative bg-white rounded-2xl shadow-2xl p-4 max-w-xs">
          <button
            (click)="closeTooltip()"
          ...
          <div class="flex items-start gap-3 pr-4">
            ...
            <div>
              <p class="font-semibold text-gray-900 mb-1">¿Quieres más información?</p>
              <p class="text-sm text-gray-600">Chatea con nosotros en WhatsApp</p>
            </div>
          </div>
          <div class="absolute -right-2 top-1/2 -translate-y-1/2 w-4 h-4 bg-white rotate-45"></div>
        </div>
      </div>
...
export class WhatsappButtonComponent {
  ...
  get whatsappUrl(): string {
    ...
    const lines = [
      ...
      '▪️ Matrículas',
      '▪️ Costos y pensiones',
      '▪️ Cupos disponibles',
      '▪️ Niveles educativos (Inicial, Primaria, Secundaria)',
      '▪️ Admisión y requisitos',
      '▪️ Otro tema',
      '',
