    ...
    <div class="max-w-6xl mx-auto px-4 py-6">
      ...
      <div *ngIf="!loading" class="space-y-6">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div *ngFor="let shift of shifts" class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
...
export class ScheduleConfigComponent implements OnInit {
  ...
  load(): void {
    this.attendanceService.getScheduleConfig().subscribe({
      next: (res: any) => {
        this.configs = (res.data || res || []).map((c: any) => ({
          ...c,
          window_start: this.toHm(c.window_start),
          late_after: this.toHm(c.late_after),
          window_end: this.toHm(c.window_end),
        }));
        this.loading = false;
      },
    ...
    for (const shift of this.shifts) {
      for (const cp of this.checkpoints) {
        ...
        this.attendanceService.updateScheduleConfig(shift, cp, payload).subscribe({
          next: () => {
            pending--;
            if (pending === 0) this.finishSave(errorMessages);
          },
          error: (err: any) => {
            pending--;
            const label = `${shift === 'manana' ? 'Manana' : 'Tarde'} - ${cp === 'entrada' ? 'Entrada' : 'Salida'}`;
            const backendMsg = err?.error?.message
              || (err?.error?.errors ? Object.values(err.error.errors).flat().join(' ') : null)
              || err?.message
              || 'Error desconocido';
            errorMessages.push(`${label}: ${backendMsg}`);
            if (pending === 0) this.finishSave(errorMessages);
          }
        });
      }
  ...
  private finishSave(errorMessages: string[]): void {
    this.saving = false;
    if (errorMessages.length === 0) {
      ...
      void fireIosSwal({
        ...
        html: errorMessages.map(m => `<div class="text-left text-sm">${m}</div>`).join(''),
